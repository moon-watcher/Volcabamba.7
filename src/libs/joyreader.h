...
// #define JOYDIR_NONE    0
// #define JOYDIR_UP      1
// #define JOYDIR_DOWN    2
// #define JOYDIR_LEFT    3
// #define JOYDIR_RIGHT   4
...
#define joy_get_vertical(JOY,ACTION)          ( joy_##ACTION##_up   ( JOY ) ? JOYDIR_UP   : ( joy_##ACTION##_down  ( JOY ) ? JOYDIR_DOWN  : 0 ) )
#define joy_get_horizontal(JOY,ACTION)        ( joy_##ACTION##_left ( JOY ) ? JOYDIR_LEFT : ( joy_##ACTION##_right ( JOY ) ? JOYDIR_RIGHT : 0 ) )
...
// #define joy_exec_horizontal(J,X,A,B,C) ( joy_##X##_left ( J ) ? A            : ( joy_##X##_right ( J ) ? B             : C            ) )
 ...
 #define joy_exec_vertical(JOY,ACTION,UP,DOWN,ELSE) ({      \
    enum joydir ret = JOYDIR_NONE;                          \
    if ( joy_##ACTION##_up ( JOY ) ) {                      \
        ret = JOYDIR_UP;                                    \
        UP                                                  \
    } else if ( joy_##ACTION##_down ( JOY ) ) {             \
        ret = JOYDIR_DOWN;                                  \
        DOWN                                                \
    } else {                                                \
        ELSE                                                \
...
#define joy_exec_horizontal(JOY,ACTION,LEFT,RIGHT,ELSE) ({  \
    enum joydir ret = JOYDIR_NONE;                          \
    if ( joy_##ACTION##_left ( JOY ) ) {                    \
        ret = JOYDIR_LEFT;                                  \
        LEFT                                                \
    } else if ( joy_##ACTION##_right ( JOY ) ) {            \
        ret = JOYDIR_RIGHT;                                 \
        RIGHT                                               \
    } else {                                                \
        ELSE                                                \
